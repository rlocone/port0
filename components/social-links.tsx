...
export function getSocialLinks(): SocialLink[] {
  ...
  try {
    ...
    if (stored) return JSON.parse(stored) ?? defaultLinks;
  } catch (e) {
...
export default function SocialLinks() {
  ...
  return (
    <div className="glass rounded-2xl p-6 glass-hover transition-all duration-500 animate-fade-in-up">
      <div className="flex items-center mb-6">
        <h2 className="text-lg font-medium text-purple-300">Connect</h2>
      </div>
        ...
        <div className="flex flex-wrap gap-3">
          {links.map((link, index) => (
            <a
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-purple-500/10 to-cyan-500/10 border border-white/10 hover:border-purple-500/30 hover:from-purple-500/20 hover:to-cyan-500/20 transition-all duration-300 group animate-scale-in"
