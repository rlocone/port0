...
const defaultLinks: SocialLink[] = [
  ...
  { id: '7', platform: 'Uptime', url: 'https://uptime.phipi.io/' },
];
  ...
  try {
    ...
    if (stored) {
      ...
      if (Array.isArray(parsed)) {
        const merged = [...parsed].map((link: SocialLink) => ({
          ...link,
          platform: typeof link?.platform === 'string' ? link.platform.replace(/^🕒\s*/, '') : link?.platform,
        }));
        for (const defaultLink of defaultLinks) {
          if (!merged.some((link: SocialLink) => link?.platform === defaultLink.platform || link?.url === defaultLink.url)) {
